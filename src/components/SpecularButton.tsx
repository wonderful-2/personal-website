...
/// 来自 React Bits 的 SpecularButton（JS 版转 TypeScript）。
/// 额外做的事：加了可选的 href（渲染成 <a>，保留锚点跳转/中键新开），
/// 以及离屏 / 切后台时暂停渲染，避免多个实例一起跑 WebGL 白白耗电。
...
export default function SpecularButton({
  ...
  const propsRef = useRef<LiveProps>({} as LiveProps);
  ...
  useEffect(() => {
    ...
    const uniforms = program.uniforms as unknown as Record<
      string,
      { value: unknown }
    >;
    ...
    // 元素离屏（导航在 md 以下会被隐藏）或标签页切到后台时不渲染
    let onScreen = true;
    ...
    const lineC = new Color(1, 1, 1);
    const baseC = new Color(0.32, 0.32, 0.32);
  ...
  const classes = `specular-button specular-button--${size}${
    className ? ` ${className}` : ""
  }`;
